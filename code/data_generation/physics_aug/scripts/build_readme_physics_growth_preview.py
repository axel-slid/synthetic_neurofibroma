...
def render_fixed_camera_growth_gif(npz_path: Path, output_path: Path, gif_frames: int, fps: int) -> None:
    ...
    eye = np.array([center[0], xyz_min[1] - 3.10, center[2] + 0.04], dtype=np.float64)
    ...
    try:
        for frame_index in sample_indices:
            ...
            scene.add(pyrender.PerspectiveCamera(yfov=np.deg2rad(43.0), znear=0.01, zfar=8.0), pose=camera_pose)
